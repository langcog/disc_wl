...
\abstract{Children may encounter many opportunities for word learning in which both a novel word and a cue to its meaning coincide in time. In some conversations, however, these sources of information are not presented simultaneously, but may instead occur in succession within a discourse.  We therefore investigated children's ability to apply their knowledge of discourse structure to disambiguate a novel word to its referent in the absence of other social cues.  In Experiment 1A, we introduced 2- to 6-year-old children and adults to two novel toys, and described each using two sentences.  Between the two sentences about one of the toys, we embedded the introduction of a novel label (e.g. ``Have you seen a toma before?'') with no cues to its meaning other that its position between the descriptions.  We then asked participants to select the referent of the label.  We found that children three years old and older and adults were more likely to attribute the label to the toy whose descriptions surrounded the naming event.  To test the extent to which participants made their selections based on temporal associations (choosing the toy described proximal to the naming event) rather than inferences about discourse structure (selecting the referent based on the naming event's position within a discourse), we ran a new group of participants in Experiment 1B who heard the novel label introduced after the two descriptions of a toy rather than between the two descriptions. Both children and adults responded close to chance with this modification, indicating that temporal proximity alone did not guide participants' selections. Together, these results suggest that continuing topic information within a discourse is a reliable cue to reference that can be used to guide listeners' inferences in inferring reference.
...
=======
    \caption{\label{fig:toys} The four pairs of toys used in Experiments 1 and 2. Each toy had a unique affordance. For example, the lid in the upper left hand corner could be popped open and closed, and the spaghetti sizer in the upper right photo could be irised open and shut.} 
>>>>>>> cda6ef91018433b3a6bf69a501bdfb767ba942a6
  \end{center} 
\end{figure}
